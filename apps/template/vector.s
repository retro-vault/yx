...
;; void vector_vertline(byte x, byte y0, byte y1, byte pattern, byte mode)
_vector_vertline::
		pop	hl		; ret addr.
		pop	bc		; x,y0
		pop	de		; y1,pattern
		...
		push	hl
		;; vertical line RAW
		;; input:	b=y0, c=x, e=y1, d=pattern (i.e. 01010101), a'=mode
		;; affects:
vector_vertline_raw::
		ld	a,e		; a=y1
...
;; void vector_horzline(byte y, byte x0, byte x1, byte pattern, byte mode)
_vector_horzline::
		pop	hl		; ret addr
		pop	bc		; c=y, b=x0
...
vhl_xor:	
		ex	af,af'
		and	e		; leave just right part of the mask
		xor	(hl)		; merge with screen
		ld	(hl),a		; and on screen
